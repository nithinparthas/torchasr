# Authors of original ASR Code:  Dirk Gütlin <dirk.guetlin@gmail.com>
#           Nicolas Barascud
# Conversion to PyTorch by: Nithin Parthasarathy
...
class Pytorch_ASR():
    ...
    def __init__(self, sfreq, cutoff=20, blocksize=100, win_len=0.5,
        ...
        if ab is None:
            yw_f = torch.tensor(np.array([0, 2, 3, 13, 16, 40,
                             np.minimum(80.0, (self.sfreq / 2.0) - 1.0),
                             self.sfreq / 2.0])) * 2.0 / self.sfreq
            yw_m = torch.from_numpy(np.array([3, 0.75, 0.33, 0.33, 1, 1, 3, 3]))
    ...
    def transform(self, raw, picks="eeg", lookahead=0.25, stepsize=32, 
        ...
        X = torch.concatenate([X,
                            torch.zeros([X.shape[0], lookahead_samples])],
                          axis=1)
...
def asr_calibrate(X, sfreq, cutoff=20, blocksize=100, win_len=0.5,
    ...
    N = int(torch.round(win_len * sfreq))
    ...
    M = linalg.sqrtm(torch.real(Uavg))
    ...
    V = Vtmp[:, torch.argsort(D)]  # I think numpy sorts them automatically

    # get the threshold matrix T
    x = torch.abs(torch.dot(V.T, X))
    offsets = torch.tensor(np.int_(np.arange(0, ns - N, np.round(N * (1 - win_overlap)))))
...
def asr_process(data, sfreq, M, T, windowlen=0.5, lookahead=0.25, stepsize=32,
    ...
    if maxdims < 1:
        maxdims = torch.round(len(data) * maxdims)

    # set initial filter conditions of none was passed
    if Zi is None:
        _, Zi = yulewalk_filter(data, ab=ab, sfreq=sfreq,
                                zi=torch.ones([len(data), 8]))

    # set the number of channels
    C, S = data.shape

    # set the number of windows
    N = torch.round(windowlen * sfreq).astype(int)
    P = torch.round(lookahead * sfreq).astype(int)

    # interpolate a portion of the data if no buffer was given
    if carry is None:
        carry = torch.tile(2 * data[:, 0],
                        (P, 1)).T - data[:, torch.mod(torch.arange(P, 0, -1), S)]
    data = torch.concatenate([carry, data], axis=-1)

    # splits = torch.ceil(C*C*S*8*8 + C*C*8*s/stepsize + C*S*8*2 + S*8*5)...
    ...
    for i in range(splits):
        ...
        i_range = torch.arange(i * S // splits,
                            torch.min([(i + 1) * S // splits, S]),
                            dtype=int)
        ...
        Xcov, cov = \
            ma_filter(N,
                      torch.reshape(torch.multiply(torch.reshape(X, (1, C, -1)),
                                             torch.reshape(X, (C, 1, -1))),
                                 (C * C, -1)), cov)

        # set indices at which we update the signal
        update_at = torch.arange(stepsize,
                              Xcov.shape[-1] + stepsize - 2,
                              stepsize)
        update_at = torch.minimum(update_at, Xcov.shape[-1]) - 1

        # set the previous reconstruction matrix if none was assigned
        if last_R is None:
            update_at = torch.concatenate([[0], update_at])
            last_R = torch.eye(C)

        Xcov = torch.reshape(Xcov[:, update_at], (C, C, -1))
        ...
        for j in range(len(update_at) - 1):
            ...
            D, V = torch.linalg.eigh(Xcov[:, :, j])

            # determine which components to keep
            keep = torch.logical_or(D < torch.sum((T @ V)**2, axis=0),
                                 torch.arange(C) + 1 < (C - maxdims))
            trivial = torch.all(keep)

            # set the reconstruction matrix (ie. reconstructing artifact
            # components using the mixing matrix)
            if not trivial:
                inv = pinv(torch.multiply(keep[:, torch.newaxis], V.T @ M))
                R = torch.real(M @ inv @ V.T)
            else:
                R = torch.eye(C)
            ...
            if (not trivial) or (not last_trivial):
                ...
                subrange = i_range[torch.arange(last_n, n)]

                # generate a cosine signal
                blend_x = torch.pi * torch.arange(1, n - last_n + 1) / (n - last_n)
                blend = (1 - torch.cos(blend_x)) / 2

                # use cosine blending to replace data with reconstructed data
                tmp_data = data[:, subrange]
                data[:, subrange] = torch.multiply(blend, R @ tmp_data) + \
                                    torch.multiply(1 - blend, last_R @ tmp_data) # noqa

            # set the parameters for the next iteration
            last_n, last_R, last_trivial = n, R, trivial

    # assign a new lookahead portion
    carry = torch.concatenate([carry, data[:, -P:]])
    carry = carry[:, -P:]
...
def clean_windows(X, sfreq, max_bad_chans=0.2, zthresholds=[-3.5, 5],
    ...
    shape_range = torch.arange(1.7, 3.5, 0.15)
    ...
    for ichan in range(nc):
        ...
        x = X[ichan, :] ** 2
        Y = torch.from_numpy(np.array([np.sqrt(np.sum(x[o:o + N]) / N) for o in offsets]))
    ...
    wz[torch.isnan(wz)] = np.inf  # Nan to inf, find pytorch conversion
    swz = torch.sort(wz, axis=0)

    # determine which windows to remove
    if torch.max(zthresholds) > 0:
        mask1 = swz[-(torch.Tensor.int(max_bad_chans) + 1), :] > torch.max(zthresholds)
    if torch.min(zthresholds) < 0:
        mask2 = (swz[1 + torch.Tensor.int(max_bad_chans - 1), :] < torch.min(zthresholds))

    # combine the two thresholds
    remove_mask = torch.logical_or.Tensor.reduce((mask1, mask2))  # check if this works
    removed_wins = torch.where(remove_mask)

    # reconstruct the samples to remove
    sample_maskidx = []
    for i in range(len(removed_wins[0])):
        if i == 0:
            sample_maskidx = torch.arange(
                offsets[removed_wins[0][i]], offsets[removed_wins[0][i]] + N)
        else:
            sample_maskidx = torch.vstack((
                sample_maskidx,
                torch.arange(offsets[removed_wins[0][i]],
                          offsets[removed_wins[0][i]] + N)
            ))

    # delete the bad chunks from the data
    def torchdelete(arr: torch.Tensor, ind: int, dim: int) -> torch.Tensor:
        skip = [i for i in range(arr.size(dim)) if i != ind]
        indices = [slice(None) if i != dim else skip for i in range(arr.ndim)]
        return arr.__getitem__(indices)


    sample_mask2remove = torch.unique(sample_maskidx)
    if sample_mask2remove.size:
        clean = torchdelete(X, sample_mask2remove, 1)       # torchdelete defined above
        sample_mask = torch.ones((1, ns), dtype=bool)
